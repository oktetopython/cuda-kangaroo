...
bool Kangaroo::MergeWorkPartPart(std::string &part1Name, std::string &part2Name)
{
  ...
  if (!partIsEmpty)
  {
    ...
    SAFE_FREAD(&RS1.bits64, 32, 1, f1);
    RS1.bits64[4] = 0;
    SAFE_FREAD(&RE1.bits64, 32, 1, f1);
    RE1.bits64[4] = 0;
    SAFE_FREAD(&k1.x.bits64, 32, 1, f1);
    k1.x.bits64[4] = 0;
    SAFE_FREAD(&k1.y.bits64, 32, 1, f1);
    k1.y.bits64[4] = 0;
  ...
  SAFE_FREAD(&RS2.bits64, 32, 1, f2);
  RS2.bits64[4] = 0;
  SAFE_FREAD(&RE2.bits64, 32, 1, f2);
  RE2.bits64[4] = 0;
  SAFE_FREAD(&k2.x.bits64, 32, 1, f2);
  k2.x.bits64[4] = 0;
  SAFE_FREAD(&k2.y.bits64, 32, 1, f2);
  k2.y.bits64[4] = 0;
  ...
  // Use modern C++ memory management with RAII - SECURITY FIX
  ...
  std::memset(params.get(), 0, nbThread * sizeof(TH_PARAM));
  uint64_t nbDP = 0;
  ...
  for (int p = 0; p < MERGE_PART && !endOfSearch; p += nbThread)
  {
    ...
    for (int i = 0; i < nbThread; i++)
    {
      ...
      params[i].hStart = p + i;
      params[i].hStop = 0;
      // Use modern C++ string management - SECURITY FIX
      params[i].part1Name = new char[part1Name.length() + 1];
      std::strcpy(params[i].part1Name, part1Name.c_str());
      params[i].part2Name = new char[part2Name.length() + 1];
      std::strcpy(params[i].part2Name, part2Name.c_str());
    ...
    for (int i = 0; i < nbThread; i++)
    {
      delete[] params[i].part1Name; // Use delete[] for new[]
      delete[] params[i].part2Name; // Use delete[] for new[]
      nbDP += params[i].hStop;
    }
  }

  // RAII: params and thHandles automatically freed by unique_ptr destructors
  // No manual free() calls needed - memory safety guaranteed
