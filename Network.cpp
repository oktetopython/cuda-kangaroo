...
#define GET(name, s, b, bl, t)                    \
  ...
  {                                               \
    PrintNetworkError("ReadError(" name ")");     \
    isConnected = false;                          \
    close_socket(s);                              \
    return false;                                 \
  }
#define PUT(name, s, b, bl, t)                      \
  ...
  {                                                 \
    PrintNetworkError("WriteError(" name ")");      \
    isConnected = false;                            \
    close_socket(s);                                \
    return false;                                   \
  }
...
bool Kangaroo::HandleRequest(TH_PARAM *p)
{
  ...
  while (p->isRunning)
  {
    ...
    case SERVER_LOADKANG:
    {
      ...
      checkSum.SetInt32(0);
      KBuff = std::make_unique<int128_t[]>(KANG_PER_BLOCK);
      ...
      while (nbKangaroo > 0)
      {
        ...
        PUTFREE("packet", p->clientSock, KBuff.get(), nbK * 16, ntimeout, KBuff.get());

        nbKangaroo -= nbK;
      }

      // KBuff automatically freed by unique_ptr destructor
    ...
    case SERVER_SAVEKANG:
    {
      ...
      while (nbKangaroo > 0)
      {
        ...
        GETFREE("packet", p->clientSock, KBuff.get(), nbK * 16, ntimeout, KBuff.get());
    ...
    case SERVER_SENDDP:
    {
      ...
      else
      {
        ...
        // Modern C++ RAII memory management for DP array
        auto dp = std::make_unique<DP[]>(head.nbDP);
        GETFREE("DP", p->clientSock, dp.get(), sizeof(DP) * head.nbDP, ntimeout, dp.get());
        state = GetServerStatus();
        PUTFREE("Status", p->clientSock, &state, sizeof(int32_t), ntimeout, dp.get());
        ...
        if (nbRead != (int)(sizeof(DP) * head.nbDP))
        {
          ...
          // Modern C++ RAII: dp automatically cleaned up when going out of scope
          CLIENT_ABORT();
        }
        else
        {
...
// #define VALIDITY_POINT_CHECK
          ...
          DP_CACHE dc;
          dc.nbDP = head.nbDP;
          dc.dp = dp.release(); // Transfer ownership to DP_CACHE
          recvDP.push_back(dc);
          UNLOCK(ghMutex);
        }
      }
...
void Kangaroo::AcceptConnections(SOCKET server_soc)
{
  ...
  while (true)
  {
    ...
    else
    {
      ...
      // Modern C++ RAII memory management for TH_PARAM
      auto p_ptr = std::make_unique<TH_PARAM>();
      TH_PARAM *p = p_ptr.get(); // Keep raw pointer for compatibility
      ::memset(p, 0, sizeof(TH_PARAM));
      char info[256];
      char addr_str[INET_ADDRSTRLEN];
      inet_ntop(AF_INET, &client_add.sin_addr, addr_str, INET_ADDRSTRLEN);
      ::snprintf(info, sizeof(info), "%s:%d", addr_str, ntohs(client_add.sin_port));
#ifdef WIN64
      ...
      // Transfer ownership to thread - thread is responsible for cleanup
      LaunchThread(_acceptThread, p_ptr.release());
    }
  }
}
...
bool Kangaroo::ConnectToServer(SOCKET *retSock)
{
  ...
  if (!hostInfo)
  {
    ...
    if (status != 0)
    {
      ...
      hostInfo.reset(); // Modern C++ way to set smart pointer to null
      hostInfoLength = 0;
      return false;
    }
    else
    {
      ...
      // Modern C++ RAII memory management for hostInfo
      hostInfo = std::make_unique<char[]>(hostInfoLength);
      ::memcpy(hostInfo.get(), &addr_in->sin_addr, hostInfoLength);
  ...
  ::memcpy((char *)&server.sin_addr, hostInfo.get(), hostInfoLength);
...
bool Kangaroo::GetKangaroosFromServer(std::string &fileName, std::vector<int128_t> &kangs)
{
  ...
  if (!endOfSearch)
  {
    ...
    while (nbKangaroo > 0)
    {
      ...
      GETFREE("packet", serverConn, KBuff.get(), nbK * 16, ntimeout, KBuff.get());
...
bool Kangaroo::SendKangaroosToServer(std::string &fileName, std::vector<int128_t> &kangs)
{
  ...
  if (!endOfSearch)
  {
    ...
    while (nbKangaroo > 0)
    {
      ...
      for (uint32_t k = 0; k < nbK; k++)
      {
        std::memcpy(&KBuff[k], &kangs[pos], 16);
        pos++;
        // Checksum
        Int K;
        K.SetInt32(0);
      ...
      PUTFREE("packet", serverConn, KBuff.get(), nbK * 16, ntimeout, KBuff.get());
...
bool Kangaroo::SendToServer(std::vector<ITEM> &dps, uint32_t threadId, uint32_t gpuId)
{
  ...
  if (!endOfSearch)
  {
    ...
    // Send DP - Modern C++ RAII memory management
    auto dp = std::make_unique<DP[]>(nbDP);
    ...
    PUTFREE("CMD", serverConn, &cmd, 1, ntimeout, dp.get());
    PUTFREE("DPHeader", serverConn, &head, sizeof(DPHEADER), ntimeout, dp.get());
    PUTFREE("DP", serverConn, dp.get(), sizeof(DP) * nbDP, ntimeout, dp.get());
    GETFREE("Status", serverConn, &status, sizeof(uint32_t), ntimeout, dp.get())
