...
struct ThreadParameters
{
  ...
  int gridSizeY;
  int gpuId;
  std::unique_ptr<GPUEngine> gpu_owner; // RAII GPU engine owner
#endif

  Int *px;       // Kangaroo position (raw pointer for compatibility)
  Int *py;       // Kangaroo position (raw pointer for compatibility)
  Int *distance; // Travelled distance (raw pointer for compatibility)

  // RAII smart pointer owners for automatic memory management
  std::unique_ptr<Int[]> px_owner;
  std::unique_ptr<Int[]> py_owner;
  std::unique_ptr<Int[]> distance_owner;

#ifdef USE_SYMMETRY
  uint64_t *symClass;                         // Last jump (raw pointer for compatibility)
  std::unique_ptr<uint64_t[]> symClass_owner; // RAII owner
#endif
...
typedef ThreadParameters TH_PARAM;
...
private:
  ...
  uint32_t dpSize;
  int32_t initDPSize;
  std::atomic<uint64_t> collisionInSameHerd{0};
  ...
  std::string serverIp;
  std::unique_ptr<char[]> hostInfo; // Modern C++ RAII memory management
  int hostInfoLength;
  int hostAddrType;
  bool clientMode;
  bool isConnected;
